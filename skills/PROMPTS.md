...
timebox: 40m
constraints:
- ISO 27001 scope strict
- Rate-limit ffuf/active scans (หลีกเลี่ยง DoS)
output_required:
1) all_subs.txt / live_hosts.txt / params.txt (Jason Haddix methodology)
2) tech_nuclei.txt
3) Top 3 attack paths พร้อม MITRE T-Codes (TA0043)
```
...
scan_policy: safe
constraints:
- Focus on finding Web/API/SMB/SSH access ports
output_required:
1) live hosts & open ports
...
- Automated pipeline rate-limt (dalfox)
deliver:
- xss_results.txt หรือ DOM payload execution evidence
...
## `priv-escalation` (`rt-access`)

```text
skill: priv-escalation
phase: access
foothold_host: <target>
constraints:
- PoC level only (ห้าม maintain persistence)
- Offline cracking สำหรับ Kerberoasting
- ขอ #approve จาก rt-lead เสมอก่อนรัน
deliver:
- PrivEsc Path (e.g. SUID, Unquoted Path) พร้อม T-Code (TA0004)
- PoC Evidence (whoami / klist)
- C-I-A Impact Assessment
```

---

## `lateral-movement` (`rt-access`)

```text
skill: lateral-movement
phase: access
foothold_host: <current_host>
credential_bundle: <hash/ticket>
constraints:
- ห้าม read/copy ข้อมูลจริง (PoC only)
- Scope strict (IP range เดิม)
- ขอ #approve ก่อน pivot ไปเครื่องใหม่
deliver:
- Access Chain & Pivots พร้อม T-Code (TA0008)
- impacket / netexec CLI evidence
- C-I-A Impact Assessment
```
...
input: evidence bundle จาก recon/webops/access
deliver:
1) Attack Narrative พร้อม MITRE T-Codes (Lifecycle)
2) OWASP Vulnerability Categorization
3) Detection Gaps Analysis ⭐ (อะไรควรเจอ -> ทำไมเรดาร์จับไม่ได้)
